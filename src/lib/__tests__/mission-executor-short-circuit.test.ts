...
describe("implicit tool params", () => {
  it("web_search 默认使用编辑框写入的 inputParams.query", () => {
    expect(
      buildImplicitWebSearchParams("A股暴跌", {
        query: "搜索A股暴跌的最新新闻、原因分析及市场反应",
        intentSummary: "备用摘要",
      }),
    ).toMatchObject({
      query: "搜索A股暴跌的最新新闻、原因分析及市场反应",
  ...
  it("web_search 没有 query 时回退到用户原始指令", () => {
    expect(
      buildImplicitWebSearchParams(
        "A股暴跌",
        { intentSummary: "备用摘要" },
