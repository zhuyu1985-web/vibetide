    ...
    execute: async (params) => {
      ...
      > = Array.isArray(params.articles)
        ? (params.articles as Array<
          ...
          >)
        : [];
      const translatableFetchStatuses = new Set(["ok", "enriched_via_bocha"]);
      const filtered = rawArticles.filter((a) => {
        // 上游 batch_deep_read 必须拿到可用正文才翻译。
        // - ok: Jina 抓到原文正文
        // - enriched_via_bocha: Jina/反爬/搜索页不可用时,Bocha 搜索补全到多源摘要正文
        // fetchStatus = "fallback_summary"(summary 兜底) / "fallback_title"
        // (只有 title) / "skipped_other"(category=other 跳过) / "failed" /
        // "enriched_via_bocha_failed" 的条目都不翻译 ——
        ...
        if (
          a.fetchStatus !== undefined &&
          !translatableFetchStatuses.has(a.fetchStatus)
        ) {
          return false;
        }
      ...
      if (filtered.length === 0) {
        const usableBodyCount = rawArticles.filter(
          (a) =>
            a.fetchStatus === undefined ||
            translatableFetchStatuses.has(a.fetchStatus),
        ).length;
        ...
        return {
          articles: [],
          note: `cross_language_rewrite 过滤后 0 条可翻译稿件 (输入 ${rawArticles.length} 条,其中拿到可用正文的 ${usableBodyCount} 条,非 other 类的 ${nonOtherCount} 条)。可能原因:1) batch_deep_read 命中反爬且 Bocha 未补全,全走 summary/title 兜底;2) topic_classifier 把所有热点归 other;3) 同时满足"可用正文+非 other+高置信度"的条目为 0。`,
        };
      }
