...
${toolList}${proficiencyGuidance}`);
  ...
  // Layer 2.1: 真实性与工具调用硬约束（全局，覆盖所有 agent 任务）
  // 针对过去出现的真实事故：web_search 未被调用、LLM 按输出模板伪造 OpenAI /
  // 苹果 WWDC 新闻与假来源、假时间、假互动指数。防止"有工具但懒得调用"的
  // 空转幻觉。本条在系统提示里置顶，位置比 SKILL.md 细节更靠前，权重更高。
  layers.push(`# 真实性与工具调用（最高优先级，禁止违背）

1. **必须真实调用工具**：任何需要外部数据（搜索、抓取、媒资检索、热榜、翻译、发布等）的工作，你 **必须** 真实调用对应工具获取数据。严禁跳过工具、凭"脑中记忆"或"按输出格式想象"编造结果。
2. **严禁伪造来源 / 时间 / 数据**：不得编造 URL、不得编造媒体名（如"华尔街日报""彭博社"）、不得编造发布时间、阅读量、互动指数、专家姓名、引述原文。
3. **SKILL.md 调用要求具有绝对优先级**：如果当前步骤注入了 SKILL.md 且其中说明需要调用某工具，你必须先调用该工具再生成结论；SKILL.md 的"应调用场景"等条目视为硬性合同。
4. **工具返回空 / 失败的处理**：若工具返回 0 结果或报错，你必须如实说明（例如"全网搜索在 24h 内无命中结果，建议扩展到 7d 或调整关键词"），绝对不得替换为训练数据里其他无关话题来凑齐【产出结果】。
5. **参数来源**：调用工具所需的 query / topic / keyword 等参数，必须来自【工作流输入参数】或【本次工作流任务】中的用户原始输入；不得用步骤名、技能描述里的字面量（如"全网搜索"）或数字输入字段（如 count=2）来当搜索词。
6. **违反后果**：伪造结果将被下游质检判定为无效产出，相关任务会被标记失败。`);
...
## 输入参数充分性（重要）
如果本步骤的质量受限于用户在启动任务时未提供的关键输入（典型如：未指定时间范围、缺失背景资料、未约束目标字数 / 受众 / 风格、未给出参考案例、渠道规范缺失等），请在【质量自评】那一行之后另起一行附：

【输入参数不足：xxx】—— 用一句话说明缺了哪个参数、对本步骤质量造成什么影响（例如"未指定覆盖时间窗，默认 24h 仅命中当日突发，错过用户可能关心的周度动态"/"未提供目标字数，正文产出约 1500 字，若需 2500 字请补充"）、并给出建议用户补充的参数名。
