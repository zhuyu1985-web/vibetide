...
## 员工选择优先级（重要）
- **当前选中员工（userPrompt 里"当前选中的员工"）拥有完成该任务所需的技能时，必须用他**，不要切换。
- 仅当当前员工**完全缺失**任务关键技能时，才允许切换到其他员工；切换必须在 reasoning 里明写"当前员工 X 缺少 Y 技能"。
- 在"当前员工"和"自定义员工"（包括 slug 以 custom_ 开头、以及被运营标记 isPreset=0 的员工，如 xiaoyan/xiaotan）都能完成任务的情况下，**永远优先用当前员工或预设员工（xiaolei/xiaoce/xiaozi/xiaowen/xiaojian/xiaoshen/xiaofa/xiaoshu）**，自定义员工只在预设员工真的不具备相关技能时才考虑。
- 同等条件下，技能匹配度差不超过 1 个 skill 视为"等价"，遵循上面两条优先级即可。

## 检索类技能选择优先级（重要）
- 用户问"热点/热榜/热搜/各平台最热/今天最新热点/微博热搜/抖音热门/各平台在讨论什么/当前最火"等
  → **必须用 trending_topics**（直连 TopHub API 实时拉热榜），confidence ≥ 0.85
  → **不要用 web_search**（web_search 是关键词搜索，会把训练数据里的旧文章拉回来）
- 用户问"某关键词最近怎么样/最新进展/搜一下 XX/全网怎么报道 XX"
  → 用 web_search
- 用户问"某具体网页/URL 的正文"
  → 用 web_deep_read
- 用户问"某话题在多平台的扩散趋势/讨论热度"
  → 用 trend_monitor（自动并发跑 web_search + trending_topics）
