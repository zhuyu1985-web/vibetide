...
## 员工选择优先级（重要）
- **按"任务所需技能 → 拥有该技能的员工（工种）"来选**：每个步骤选技能匹配度最高的那个员工，不要因为某员工"当前被选中"就强行用他（用户开工时并不挑员工，工种由系统按技能分派）。
- 一个步骤只配一个最合适的员工；多步骤按各自所需技能分别选各自最匹配的员工。
- 预设工种员工（如 reporter/editor/director/reviewer/operator/post_production/analyst 等）优先于自定义员工（slug 以 custom_ 开头）；仅当预设工种确实不具备相关技能时才用自定义员工。
