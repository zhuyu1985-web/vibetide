...
export async function testSkillExecution(
  ...
  if (isToolRegistered(skill.name)) {
    ...
    return {
      ...
      outputSchema,
      runtimeInfo: {
        type: `Tool (真实接口${writeTool ? " · dryRun" : ""})`,
      ...
      expectedBehavior: invocation.ok
        ...
        : `[真实调用失败] ${skill.name}: ${invocation.error}`,
      executionResult: {
        success: invocation.ok,
        output: truncated,
        error: invocation.ok ? undefined : invocation.error,
        durationMs,
      },
      validationChecks: [
        {
          ...
          detail: `tool-registry 命中 ${skill.name}${writeTool ? "(自动 dryRun 防污染)" : ""}`,
        },
        {
          check: "参数校验",
  ...
  // Resolve model: provider from env (LLM_PROVIDER), model from getDefaultModel()
  ...
  try {
    ...
    const { getLanguageModel, getDefaultModel } = await import("@/lib/agent/model-router");
