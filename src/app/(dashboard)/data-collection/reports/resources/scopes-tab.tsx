...
import {
  setDefaultMediaScope,
  deleteMediaScopeAction,
} from "@/app/actions/research/media-scopes";
...
export function ScopesTab({ rows }: Props) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const [uploadOpen, setUploadOpen] = useState(false);
  const [detailScopeId, setDetailScopeId] = useState<string | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<ScopeRow | null>(null);

  function handleSetDefault(scopeId: string) {
    startTransition(async () => {
      try {
        await setDefaultMediaScope(scopeId);
        toast.success("已设为默认名单");
        router.refresh();
      } catch (err) {
  ...
  function handleDelete(force: boolean) {
    ...
    startTransition(async () => {
      try {
        await deleteMediaScopeAction(id, force);
        toast.success("名单已删除");
        router.refresh();
      } catch (err) {
    ...
    },
    {
      ...
      render: (r) => (
        <span className="text-xs text-muted-foreground tabular-nums">
          央 {r.centralCount} · 业 {r.industryCount} · 市 {r.municipalCount} · 融{" "}
          {r.districtRmtCount} · 政 {r.districtGovCount}
        </span>
      ),
    },
    {
      ...
      align: "right",
      render: (r) => (
        <span className="text-xs tabular-nums text-muted-foreground">{r.totalUnits}</span>
      ),
    },
    {
      key: "sourceFileName",
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        ...
        emptyMessage={
          <div className="py-8 space-y-2">
            <div className="text-sm text-muted-foreground">暂无媒体名单</div>
            <div className="text-xs text-muted-foreground">
              点击右上角「上传新名单」开始,支持 xlsx 格式
            </div>
          </div>
        }
      />
      ...
      {detailScopeId && (
        <ScopeDetailDrawer scopeId={detailScopeId} onClose={() => setDetailScopeId(null)} />
      )}
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>删除媒体名单</DialogTitle>
            <DialogDescription>
              {deleteTarget
                ? `确定删除「${deleteTarget.name}」?关联的 ${deleteTarget.totalUnits} 个单位将一并清理。`
                : ""}
            </DialogDescription>
          </DialogHeader>
          <p className="text-xs text-muted-foreground">
            若该名单已被报告引用,普通删除会被拒绝。如确认要删除,请用「强制删除」(已生成的报告快照会保留,但下次重生成会失败)。
          </p>
