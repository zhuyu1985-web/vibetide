...
description: 为文章 / 稿件 / 视频 / 播客 / 专题自动生成摘要，同时产出三种形态供不同渠道使用：CMS 详情页摘要（≤ 200 字，保留事实密度和观点）、社交分享摘要（≤ 80 字，含钩子和行动引导）、APP 列表页卡片摘要（≤ 50 字，一句话抓人）。按 9 大 APP 栏目场景分化摘要风格（新闻事实型 / 时政严肃 / 体育情绪 / 综艺轻松 / 民生亲切 / 短剧悬念 / 每日专题概览）。从正文自动抽取核心信息 + LLM 润色 + 多版本 A/B 对比 + 字数硬校验 + 去主观臆断扫描 + 关键数据点保留检查。支持手动指定"必保留点"（如特定人名 / 机构 / 数字）。当用户提及"出摘要""简述""summary""卡片文案""分享文""一句话概括"等关键词时调用；不用于完整正文生成或标题生成。
version: 5.0.0
