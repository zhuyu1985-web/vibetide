...
// ---------------------------------------------------------------------------
// 27 Employee Scenarios — all 8 AI employees
// ---------------------------------------------------------------------------
...
async function seedScenarios() {
  ...
  for (const slug of slugs) {
    ...
    // Delete existing scenarios for this employee+org (idempotent)
    await db
      .delete(schema.employeeScenarios)
      .where(
        and(
          eq(schema.employeeScenarios.organizationId, org.id),
          eq(schema.employeeScenarios.employeeSlug, slug)
        )
      );
    ...
    console.log(`  ${slug}: ${scenarios.length} scenarios seeded`);
    for (const s of scenarios) {
