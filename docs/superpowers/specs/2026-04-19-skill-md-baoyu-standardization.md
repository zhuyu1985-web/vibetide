...
metadata:
  ...
  modelDependency: deepseek:deepseek-chat            # 项目默认；生成类 skill 需要，管理/检索类可省。若要 Opus 填 anthropic:claude-opus-4-7
  requires:
...
**本 spec 要求**（I1 修正）：为 3 个 script-heavy skill（duanju / zhongcao / podcast）**创建 stub 文件**（`src/lib/agent/skills/<name>-subtemplates.ts`），内容为 10 行 TS stub（`export const SUBTEMPLATES = {} as const`）+ TODO 注释。这样 §10 AC 第 2 条 `scriptPath` 存在性检查能通过，真实填充内容由 follow-up PR 完成（tracker: "TBD issue" — 合入 main 后开一个 GitHub issue 跟踪）。

stub 模板（每个 skill 都用这个）：
```ts
/**
 * Subtemplates for <skill_name>.
 *
 * TODO(follow-up): populate subtemplate detailed specs extracted from SKILL.md §4.4
 * See Track B spec: docs/superpowers/specs/2026-04-19-skill-md-baoyu-standardization.md §4.4
 */
...
1. **P0 删（必删）：**
   ...
   - 所有 Changelog（N2 例外：若 Changelog 含 breaking change migration 指南而非单纯版本号，把 migration 段**迁移到 referenceSpec 或 new appendix**，不直接删）
...
- **真实填充** `src/lib/agent/skills/<name>-subtemplates.ts` 的业务数据（本 spec 只要求 stub，真实填充放 **follow-up issue**）
  - follow-up tracker: 合入 main 后开一个 GitHub issue "TBD: populate script-heavy subtemplates"，关联本 spec §4.4
...
- [ ] 总行数从 7797 降到 ≤ 3500（目标 3280，允许 7% buffer）
- [ ] 3 个 script-heavy skill stub 文件已创建：`src/lib/agent/skills/{duanju,zhongcao,podcast}-subtemplates.ts`，每个 10 行 TS stub + TODO 注释（真实填充放 follow-up issue）
...
- 批 3（4 个 script-heavy）：每个 ~60-90 分钟（N3 修正：duanju 1211→300 要重写 12 子模板外置，比原估乐观；单独 40 分钟不够），总 240-360 分钟
- 3 个 subtemplates stub 文件 + 验收 + commit：45 分钟

**总预计：** **8-9 小时**（1+ 天工作量）

subagent-driven 并行化：批 1/2 适合并行（风格独立），批 3 不建议并行（子模板外置需要一致性把关）。并行化后预计 5-6 小时。
