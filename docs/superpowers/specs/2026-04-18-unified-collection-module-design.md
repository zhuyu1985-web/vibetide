...
Vibetide 现有 **9 个数据采集入口**分散在代码库里，每个模块（热榜、研究、对标、知识库等）各自实现抓取、重试、去重、日志——导致：
...
- 7 个现有采集入口全部迁移（灵感池 / 热榜 cron / 对标 / 研究任务 3 分支 / 知识库 URL）+ 热榜富化改为订阅事件
- 运营后台 3 个页面：源管理、内容浏览、监控面板
- Trigram 全文搜索（避免 Supabase 扩展依赖）
- 内容浏览卡片 + 表格双视图
...
- "我的订阅"式个性化推送
- 跨租户共享源库
- Agent 对话工具里的 Tavily 搜索（临时查询，不落盘，不接入此模块）
...
export const collectedItems = pgTable("collected_items", {
  ...
  // tsvector 全文搜索列（通过 generated column 自动维护）
  // CREATE COLUMN search_tsv tsvector GENERATED ALWAYS AS (...) STORED;
  // + trigram index on (title || ' ' || coalesce(content,''))
...
### 6.3 去重策略
- **主指纹**：`MD5(normalize(title) + date_bucket_24h(publishedAt ?? now()))`
  - `normalize(title)` = 去标点、繁简转简体、去空白、小写
  - `date_bucket_24h` = UTC 当天 0 点的 epoch
- **辅助指纹**：`MD5(canonical_url)` 当有规范 URL 时
- **合并逻辑**：
  1. 先按 `canonical_url_hash` 查找，有 → 合并
  2. 再按 `content_fingerprint` 查找，有 → 合并
...
async function writeItems({ runId, sourceId, organizationId, items }: WriteArgs) {
  let inserted = 0, merged = 0, failed = 0;

  for (const raw of items) {
    try {
      const { titleNorm, urlCanon } = normalize(raw);
      const fingerprint = computeFingerprint(titleNorm, raw.publishedAt);
      ...
      const itemId = await db.transaction(async (tx) => {
        // 先按 URL 查
        const byUrl = urlHash
          ? await tx.select().from(collectedItems)
              .where(and(eq(orgId), eq(canonicalUrlHash, urlHash)))
              .forUpdate()
          : null;
        if (byUrl) {
        ...
        const byFp = await tx.select().from(collectedItems)
          .where(and(eq(orgId), eq(contentFingerprint, fingerprint)))
          .forUpdate();
        if (byFp) {
      ...
      if (isNew) {
        await inngest.send({
          ...
          data: { itemId, sourceId, organizationId, targetModules: source.targetModules },
        });
      }
    } catch (err) {
      failed++;
