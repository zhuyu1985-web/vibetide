...
> 本文件为 Phase 1 CMS Adapter MVP Task 37 的 E2E 验证报告模板。实际执行时请将各占位符替换为真实值，并根据"验证清单"逐项勾选，然后把"问题与修正"栏补齐。
...
| `<YOUR_ARTICLE_ID>` | 每次执行 Step 37.3 后 console.log 的返回值 |
| `<CMS_ARTICLE_ID_FROM_STEP_37_4>` | Step 37.4 的 `result.cmsArticleId` |
| `<picked_catalog_uuid>` | Step 37.2 查 `cms_catalogs` 挑一条的 id |
